...
const index = () => {
  ...
  useEffect(()=>{
    const fetchdata=async()=>{
      try {
        const res=await axios.get( "https://internshala-c.onrender.com/api/job")     
        setjob(res.data)
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="hidden md:block w-64 bg-white rounded-lg shadow-sm p-6 h-fit">
          ...
          <div className="space-y-6">
            ...
            {/* Salary Range */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Annual Salary (₹ in lakhs)</label>
              <input
                type="range"
                name="salary"
                min="0"
        ...
        <div className="flex-1">
          ...
          <div className="bg-white p-4 rounded-lg shadow-sm mb-4">
            <p className="text-center font-medium text-black">{filteredjob.length} Jobs found</p>
          </div>
          ...
          <div className="space-y-4">
            {filteredjob.map((job: any) => (
              ...
              >
                ...
                <p className="text-gray-600 mb-2">{job.aboutJob}</p>

                <div className="flex flex-wrap gap-2 mb-4">
                  {Array.isArray(job.skills)
                    ? job.skills.map((skill: string, idx: number) => (
                        <span key={idx} className="bg-[#e2f3fa] text-[#008bdc] px-3 py-1 rounded text-sm">{skill}</span>
                      ))
                    : (job.skills || "").split(",").map((skill: string, idx: number) => (
                        <span key={idx} className="bg-[#e2f3fa] text-[#008bdc] px-3 py-1 rounded text-sm">{skill.trim()}</span>
                      ))}
                </div>

                <ul className="list-disc list-inside text-gray-600 mb-4">
                  {Array.isArray(job.perksBenefits)
                    ? job.perksBenefits.map((perk: string, idx: number) => <li key={idx}>{perk}</li>)
                    : (job.perksBenefits || "").split(",").map((perk: string, idx: number) => (
